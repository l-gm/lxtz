	...
	<div class="main container">
		...
		<div class="crumbs">
			<p>
				...
				<a href="${ctx}/portal/partTime/merchantJob">招聘信息</a>&nbsp;&gt;&nbsp;
				招聘信息列表
			</p>
		</div>
		<!-- 面包屑 end -->
				...
				<div class="e_wrapper">
					...
					<div class="e_search_bar">
						<form class="clearfix" id="form">
							<input class="s_txt fl" type="text" placeholder="按标题、内容查找" name="searchMerchantJob" value="${page.vo.jobTitle}">
						...
						<div class="em_all">
							...
							<ul>
								<li class="li_head">
									...
									<span class="em_area">工作区域</span>
									<span class="em_date">发布时间</span>
								</li>
								<c:forEach items="${page.content}" var="_item">
									<li>
										<a href="${ctx}/portal/partTime/merchantJob/view/${_item.id}">
											<span class="em_title">${_item.jobTitle}</span>
											<span class="em_area">${_item.workArea}</span>
											<span class="em_date"><fmt:formatDate value="${_item.createdDate}" pattern="yyyy年dd月mm日 HH:mm:ss" /></span>
										</a>
									</li>
								</c:forEach>
							</ul>
